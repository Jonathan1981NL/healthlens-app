// HealthLens 1I-refresh-safe-20260705
window.addEventListener('pageshow', function() {
  const badge = document.querySelector('.version-pill');
  if (badge) badge.textContent = 'Actieve versie: 1I-refresh-safe-20260705';
});

const $ = s => document.querySelector(s);
const $$ = s => Array.from(document.querySelectorAll(s));
const STORE_KEY = 'healthlens.profile.1i';
...
function renderAtlas(){
  ...
  img.src=`${layer.image}?v=1I-refresh-safe-20260705`;
  img.alt=layer.label;
...
function bind(){
  ...
  $('#latestBtn').onclick=async()=>{await unregisterOldCaches(); alert('Oude caches zijn gewist. Open opnieuw met ?v=1I-refresh-safe-20260705-latest als je telefoon oude inhoud toont.');};
}
async function init(){
...
document.addEventListener('DOMContentLoaded', function(){
  const hard = document.getElementById('hardRefreshBtn');
  if (hard) hard.addEventListener('click', async function(){
    try {
      if ('serviceWorker' in navigator) {
        const regs = await navigator.serviceWorker.getRegistrations();
        for (const reg of regs) await reg.unregister();
      ...
      localStorage.setItem('healthlens.lastForcedRefresh','1I-refresh-safe-20260705');
    } catch(e) {}
    location.href = 'stage1i.html?v=' + Date.now();
  });
});
